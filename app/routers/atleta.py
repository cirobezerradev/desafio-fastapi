...
async def create_atleta(atleta: AtletaIn, session: Annotated[AsyncSession, Depends(get_session)]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_303_SEE_OTHER, detail=f'Já existe um atleta cadastrado com o cpf: {atleta.cpf}'
        )

    except SQLAlchemyError as e:
        await session.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Erro ao criar atleta: {str(e)}')
    return atleta


@router.get('/', response_model=list[AtletaOut], status_code=status.HTTP_200_OK)
async def list_atletas(
    session: Annotated[AsyncSession, Depends(get_session)], nome: str | None = None, cpf: str | None = None
):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Problema ao listar Atletas: {str(e)}')


@router.get('/{id}', response_model=list[AtletaOut], status_code=status.HTTP_200_OK)
async def get_atleta_id(id: int, session: Annotated[AsyncSession, Depends(get_session)]):
...
async def update_atleta(
    id: int, atleta_update: AtletaUpdate, session: Annotated[AsyncSession, Depends(get_session)]
) -> None:
    ...
    if not atleta:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Atleta não encontrado no is: {id}')
...
async def delete_atleta(id: int, session: Annotated[AsyncSession, Depends(get_session)]):
